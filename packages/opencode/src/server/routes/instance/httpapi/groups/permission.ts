...
const SessionIncarnation = Schema.String.check(Schema.isPattern(/^session_incarnation_[0-9a-f]{32}$/))
    ...
    HttpApiGroup.make("permission")
      .add(
        ...
        HttpApiEndpoint.post("hostedReply", "/experimental/agent-teams/hosted-approval/session/:sessionID/permission/:requestID/reply", {
          ...
          error: [HttpApiError.BadRequestNoContent, HttpApiError.ConflictNoContent, HttpApiSchema.NoContent.pipe(HttpApiSchema.status(412)), HttpApiError.NotFoundNoContent],
        }),
