...
export interface Interface {
  ...
  /** Call only while holding withConditionalReply, in the synchronous policy commit. */
  readonly invalidatePolicy: () => void
        ...
        withConfigMutation: (effect, changed = () => true) =>
          Effect.uninterruptibleMask((restore) =>
            Effect.gen(function* () {
              ...
              const result = yield* restore(effect).pipe(
                Effect.onExit((exit) => Exit.isFailure(exit)
                  ? mutex.withPermit(Effect.sync(() => service.invalidatePolicy()))
                  : Effect.void),
              )
              if (changed(result)) {
