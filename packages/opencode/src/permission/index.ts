    ...
    const reply = Effect.fn("Permission.reply")((input: PermissionV1.ReplyInput) => {
      const settle = mutex.withPermit(
        Effect.gen(function* () {
          ...
          // Commit the entire decision before invoking fallible, interruptible listeners.
          // No yield may separate policy invalidation, removal, and waiter settlement.
          const settled = yield* Effect.sync(() => {
            if (input.reply === "always") {
              hostedApproval.invalidatePolicy()
              for (const pattern of existing.info.always) {
                approved.push({ permission: existing.info.permission, pattern, action: "allow" })
              }
            }
            const settled = [existing, ...Array.from(pending.values()).filter((item) => {
              if (item === existing || item.info.sessionID !== existing.info.sessionID) return false
              if (input.reply === "reject") return true
              return input.reply === "always" && item.info.patterns.every(
                (pattern) => evaluate(item.info.permission, pattern, approved).action === "allow",
              )
            })]
            for (const item of settled) {
              pending.delete(item.info.id)
              Deferred.doneUnsafe(item.deferred, input.reply === "reject"
                ? Effect.fail(item === existing && input.message
      ...
      return hostedApproval.withConditionalReply(settle)
    })
        ...
        mutex.withPermit(
          Effect.gen(function* () {
            ...
            if (provenance && !operationNonce) {
              provenance.poison("conditional-reply-operation-nonce-missing")
            }
            // Settlement and durable capture precede all fallible listener publication.
            // Capture failure poisons the producer; the committed decision cannot be retried.
            yield* Effect.sync(() => {
              pending.delete(input.requestID)
              Deferred.doneUnsafe(existing.deferred, input.reply === "once"
                ? Effect.void
                : Effect.fail(input.message
                  ? new PermissionV1.CorrectedError({ feedback: input.message })
                  : new PermissionV1.RejectedError()))
              if (provenance && operationNonce) {
                provenance.emit("protectedEffectLedger", {
              ...
              provenance?.poison("conditional-reply-capture-failed")
            })))
            yield* events.publish(Event.Replied, {
              sessionID: existing.info.sessionID,
              requestID: existing.info.id,
