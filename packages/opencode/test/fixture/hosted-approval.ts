...
export function app(password?: string, producer?: Producer) {
  ...
  const provenanceLayer = HostedApprovalProvenance.makeLayer(producer ?? null)
  const runtime = ManagedRuntime.make(AppNodeBuilderV1.build(
    LayerNode.group([Permission.node, InstanceStore.node, EventV2Bridge.node]),
    [[HostedApprovalProvenance.node, provenanceLayer]],
  ).pipe(Layer.provideMerge(AppNodeBuilderV1.build(Ripgrep.node)), Layer.provideMerge(Observability.layer)), { memoMap })
  const web = HttpRouter.toWebHandler(
    (producer
      ? HttpApiApp.createRoutes(undefined, provenanceLayer)
  ...
  const operations: Operations = {
    ...
    sync: (fd) => {
      ...
      for (const line of lines) {
        ...
        if (record.recordType === "producer-close") closes.push(fd === 9 ? "openCodeTimeline" : "protectedEffectLedger")
