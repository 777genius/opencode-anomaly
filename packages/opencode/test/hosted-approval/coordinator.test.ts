...
test("failed mutation work conservatively invalidates a possibly committed policy", async () => {
  const result = await Effect.gen(function* () {
    const coordinator = yield* HostedApprovalCoordinator.Service
    const before = coordinator.snapshot().configGeneration
    yield* coordinator.withConfigMutation(Effect.die("failure after an unknown commit boundary")).pipe(Effect.exit)
    return { before, after: coordinator.snapshot().configGeneration }
  }).pipe(Effect.provide(HostedApprovalCoordinator.makeTestLayer()), Effect.runPromise)
  expect(result.after).not.toBe(result.before)
})
