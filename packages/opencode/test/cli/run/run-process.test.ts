...
import {
  ...
  portableControllerPublicationWindowForTest,
  ...
  portableMarkerPollingTeardownForTest,
  ...
  portableLinuxZombieGroupDrainForTest,
  portableLinuxNonLeaderZombieGroupDrainForTest,
  portablePersistentControllerRecoveryForTest,
  portableStalledControllerRecoveryForTest,
  ...
  portableChildrenOverflowForTest,
  portableChunkedZombieLineageForTest,
  portableOwnedZombieLineageForTest,
  portableOwnedUnreadableStatForTest,
  portableParentPIDReuseForTest,
  portableParentTaskDisappearanceForTest,
  portableParentTaskUnreadableForTest,
  portablePreIdentityLineageFailureForTest,
  portableUnrelatedStatRaceForTest,
  terminationStepFailuresForTest,
...
describe("CLI process cleanup containment", () => {
  ...
  test("portable controller admission keeps its acquisition window for delayed signed publication", async () => {
    if (process.platform === "win32") return
    ...
    expect(result.admissionDurationMs).toBeGreaterThanOrEqual(400)
    expect(result.admissionDurationMs).toBeLessThan(4_000)
  })

  test("portable controller keeps the full publication window after the predecessor cleanup cutoff", () => {
    if (process.platform === "win32") return
    const result = portableControllerPublicationWindowForTest()
    expect(result.publishedAt).toBeGreaterThan(result.predecessorCleanupCutoff)
    expect(result.publishedAt).toBeLessThan(result.acquisitionDeadline)
    expect(result.observationDeadline).toBeGreaterThan(result.publishedAt)
    expect(result.observationDeadline - result.publishedAt).toBe(250)
    expect(result.observationDeadline).toBeLessThanOrEqual(result.acquisitionDeadline)
  })
  ...
  test("portable controller startup failure reaps the owned gate without accepting a missing controller", async () => {
    ...
    expect(result.reaped).toBe(true)
    expect(result.groupDrained).toBe(true)
    expect(result.controlsRemoved).toBe(true)
  ...
  test("portable marker polling tears down an early-exited target and its controls", async () => {
    if (process.platform === "win32") return
    const result = await portableMarkerPollingTeardownForTest()
    expect(formatProcessErrorForTest(result.cause)).toContain("exited before publishing")
    expect(result.reaped).toBe(true)
    expect(result.groupDrained).toBe(true)
    expect(result.controlsRemoved).toBe(true)
  })
  ...
  test("successful portable cleanup waits for controller KILL and removes acknowledged controls", async () => {
    ...
    expect(result.durationMs).toBeLessThan(2_500)
    // The gate consumes and removes the release control before exec; the
    // finalizer removes the six controls still owned after acknowledgement.
  ...
  test("portable persistent controllerStatus failures use nonce discovery without spending finalization reserve", async () => {
    ...
    for (const kind of ["read", "authenticate"] as const) {
      ...
      expect(result.controllerGone).toBe(true)
      expect(result.rootReaped).toBe(true)
      if (kind === "authenticate") expect(result.durationMs).toBeLessThan(3_000)
    }
  })

  test("portable stalled signed-controller observation preserves finalization reserve", async () => {
    if (process.platform === "win32") return
    const result = await portableStalledControllerRecoveryForTest()
    expect(result.cause).toBeInstanceOf(AggregateError)
    expect(formatProcessErrorForTest(result.cause)).toContain("portable initial observation exceeded its deadline")
    expect(result.attempts).toBeGreaterThan(1)
    expect(result.fallback).toBe(true)
    expect(result.controllerGone).toBe(true)
    expect(result.rootReaped).toBe(true)
  ...
  test("portable group drain reaps an owned zombie without reading unrelated executable paths", async () => {
    if (process.platform !== "linux") return
    const result = await portableLinuxZombieGroupDrainForTest()
    expect(result.unrelatedStatRead).toBe(true)
    expect(result.unrelatedExecutablePaths).toEqual([])
    expect(result.drainWhileZombie).toBe(false)
    expect(result.trackedZombie).toBe(true)
    expect(result.reaped).toBe(true)
    expect(result.drainedAfterCleanup).toBe(true)
    expect(result.cleanupDurationMs).toBeLessThan(3_000)
  })

  test("portable discovery drains a private-group zombie forked by a non-leader task", async () => {
    if (process.platform !== "linux") return
    const result = await portableLinuxNonLeaderZombieGroupDrainForTest()
    expect(result.drainWhileZombie).toBe(false)
    expect(result.trackedZombie).toBe(true)
    expect(result.reaped).toBe(true)
    expect(result.drainedAfterCleanup).toBe(true)
  ...
  test("portable discovery ignores an unrelated stat which becomes unreadable after enumeration", async () => {
    if (process.platform !== "linux") return
    await expect(portableUnrelatedStatRaceForTest()).resolves.toEqual({
      snapshots: [],
      errors: [],
      complete: true,
      statReads: 0,
  ...
  test("portable discovery retains an owned unreadable stat as a permanent diagnostic", async () => {
    if (process.platform !== "linux") return
    const result = await portableOwnedUnreadableStatForTest()
    expect(result).toMatchObject({
      snapshots: [],
      complete: true,
    })
    expect(result.errors.map((error) => error.message)).toEqual(["failed to snapshot nonce-authorized process 4242"])
    expect(result.errors[0]?.cause).toBeInstanceOf(Error)
  })

  test("portable discovery does not adopt a child from a reused parent PID", async () => {
    if (process.platform !== "linux") return
    await expect(portableParentPIDReuseForTest()).resolves.toMatchObject({
      snapshots: [{ pid: 42 }],
      errors: [],
      complete: true,
      childSnapshotAttempts: 1,
      childStatReads: 3,
      parentBracketReads: 2,
    })
  })

  test("portable discovery retains a previously unseen owned zombie by stable parent lineage", async () => {
    if (process.platform !== "linux") return
    await expect(portableOwnedZombieLineageForTest()).resolves.toEqual({
      snapshots: [
        {
          pid: 42,
          parent: 1,
  ...
  test("portable discovery reads short child-list chunks through the final private-group zombie", async () => {
    if (process.platform !== "linux") return
    await expect(portableChunkedZombieLineageForTest()).resolves.toEqual({
      snapshots: [
        {
          pid: 42,
          parent: 1,
  ...
  test("portable discovery rejects an overflowing child list before admitting partial children", async () => {
    if (process.platform !== "linux") return
    const result = await portableChildrenOverflowForTest()
    expect(result).toMatchObject({
      snapshots: [{ pid: 42 }],
      complete: false,
      reads: 2,
      childRecordAttempts: 0,
    })
    expect(result.errors.map((error) => error.message)).toEqual([
      "failed to inspect children of nonce-authorized process 42",
    ])
    expect(result.errors[0]?.cause).toBeInstanceOf(Error)
  })

  test("portable discovery rejects a failed child stat after stable lineage selection", async () => {
    if (process.platform !== "linux") return
    await expect(portablePreIdentityLineageFailureForTest()).resolves.toEqual({
      snapshots: [
        {
          pid: 42,
          parent: 1,
      ...
      childSnapshotAttempts: 0,
    })
  })

  test("portable discovery fails closed when an owned parent loses a task during child enumeration", async () => {
    if (process.platform !== "linux") return
    await expect(portableParentTaskDisappearanceForTest()).resolves.toEqual({
      snapshots: [
        {
          pid: 42,
          parent: 1,
  ...
  test("portable discovery fails closed when an owned parent task is unreadable", async () => {
    if (process.platform !== "linux") return
    const result = await portableParentTaskUnreadableForTest()
    expect(result).toMatchObject({
      snapshots: [{ pid: 42 }],
      complete: false,
      childRecordAttempts: 0,
    })
    expect(result.errors.map((error) => error.message)).toEqual(["failed to inspect children of nonce-authorized process 42"])
  })
  ...
  test("termination aggregates synchronous and rejected scan, signal, and stop failures after escalation", async () => {
    for (const mode of ["sync", "reject"] as const) {
      const result = await terminationStepFailuresForTest(mode)
      expect(result.exitObserved).toBe(true)
      expect(result.reaped).toBe(true)
      expect(result.stopped).toBe(true)
      expect(result.signals).toContain("SIGTERM")
      expect(result.signals).toContain("SIGKILL")
      expect(result.cause).toBeInstanceOf(AggregateError)
      const formatted = formatProcessErrorForTest(result.cause)
      expect(formatted).toContain(`synthetic scan ${mode} failed`)
      expect(formatted).toContain(`synthetic SIGTERM signal ${mode} failed`)
      expect(formatted).toContain(`synthetic SIGKILL signal ${mode} failed`)
      expect(formatted).toContain(`synthetic tracker stop ${mode} failed`)
    }
  })
