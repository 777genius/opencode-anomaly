...
import { createAcpClient } from "./helpers"
...
for (const respond of [true, false]) {
  it.effect(`ACP RPC budget starts after transport readiness (${respond ? "response" : "deadline"})`, () =>
    Effect.gen(function* () {
      ...
      const request = yield* Effect.gen(function* () {
        const client = yield* createAcpClient({
          opencode: {
            acp: () => Effect.succeed({
              ready: Deferred.await(ready),
              send: (message) => Deferred.succeed(sent, message).pipe(Effect.asVoid),
      ...
      // Cold startup exceeds the entire RPC budget, but no request has been sent.
      yield* TestClock.adjust("20 seconds")
      expect(yield* Deferred.isDone(sent)).toBe(false)
      expect(request.pollUnsafe()).toBeUndefined()
      yield* Deferred.succeed(ready, undefined)
      expect(yield* Deferred.await(sent)).toMatchObject({ method: "initialize", id: 1 })
      ...
      expect(request.pollUnsafe()).toBeUndefined()
      if (respond) {
        yield* Queue.offer(responses, { jsonrpc: "2.0", id: 1, result: {} })
