  ...
  return Effect.gen(function* () {
    ...
    const startRun = Effect.fn("opencode.startRun")(function* (message: string, opts?: RunOpts) {
      ...
      const proc = yield* Effect.acquireRelease(
        Effect.sync(() => {
          const child = Bun.spawn(["bun", "run", cliEntry, ...runArgs(message, opts)], {
            cwd: home,
    ...
    const serve = Effect.fn("opencode.serve")(function* (opts?: ServeOpts) {
      ...
      const proc = yield* Effect.acquireRelease(
        Effect.sync(() => {
          const child = Bun.spawn(["bun", "run", cliEntry, ...argv], {
            cwd: home,
    ...
    const acp = Effect.fn("opencode.acp")(function* (opts?: AcpOpts) {
      ...
      const proc = yield* Effect.acquireRelease(
        Effect.sync(() => {
          const child = Bun.spawn([process.execPath, "run", cliEntry, ...argv], {
            cwd: opts?.cwd ?? home,
