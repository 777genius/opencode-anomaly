...
const linuxProcChildrenReadBytes = 64 * 1024
const linuxProcTaskLimit = 4_096
...
type LinuxProcReader = {
  ...
  readonly record: (pid: number) => LinuxProcessRecord | undefined
  readonly children: (parent: ProcessSnapshot) => readonly number[] | undefined
  readonly hasNonce: (pid: number, nonce: string) => boolean
  readonly trustedSnapshot: (
    pid: number,
...
type LinuxProcObservation = Pick<LinuxProcReader, "record"> & {
  readonly snapshot: (pid: number, includeZombie?: boolean) => ProcessSnapshot | undefined
}

function linuxProcessRecord(
  pid: number,
  read: (file: string, encoding: "utf8") => string = (file, encoding) => readFileSync(file, encoding),
): LinuxProcessRecord | undefined {
  let stat: string
  try {
  ...
  if (!Number.isSafeInteger(parent) || parent < 0 || !Number.isSafeInteger(group) || group < 0)
    throw new Error(`process ${pid} has an invalid stat record`)
  return { parent, group, started: `${linuxBootIdentity}:${fields[19]}`, state: fields[0] }
}

function linuxBoundedRead(file: string) {
  const descriptor = openSync(file, "r")
  try {
    return linuxBoundedReadFrom(file, (bytes, offset, length) =>
      readSync(descriptor, bytes, offset, length, null),
    )
  } finally {
...
function linuxBoundedReadFrom(
  file: string,
  read: (bytes: Uint8Array, offset: number, length: number) => number,
) {
  const bytes = new Uint8Array(linuxProcChildrenReadBytes)
  let length = 0
  while (true) {
    const overflow = length === bytes.byteLength
    const target = overflow ? new Uint8Array(1) : bytes
    const offset = overflow ? 0 : length
    const readLength = read(target, offset, target.byteLength - offset)
    if (!Number.isSafeInteger(readLength) || readLength < 0 || readLength > target.byteLength - offset)
      throw new Error(`process children record ${file} returned an inconsistent read length`)
    if (readLength === 0) return new TextDecoder().decode(bytes.subarray(0, length))
    if (overflow) throw new Error(`process children record ${file} exceeds its read limit`)
    length += readLength
  }
}

function linuxProcessChildren(parent: ProcessSnapshot) {
  const before = linuxProcessRecord(parent.pid)
  if (!before || !matchesLinuxIdentity(before, parent)) return
  let tasks: readonly string[]
  try {
    tasks = readdirSync(`/proc/${parent.pid}/task`)
  } catch (cause) {
    if (isMissingProcess(cause)) return
    throw new Error(`failed to enumerate tasks for process ${parent.pid}`, { cause })
  }
  const taskIDs = tasks.flatMap((task) => {
    if (!/^\d+$/.test(task)) return []
    const id = Number(task)
    if (!Number.isSafeInteger(id) || id <= 0) return []
    return [id]
  })
  if (taskIDs.length > linuxProcTaskLimit)
    throw new Error(`process ${parent.pid} reported too many tasks for bounded child discovery`)
  const children = new Set<number>()
  for (const task of taskIDs) {
    let listed: string
    try {
      listed = linuxBoundedRead(`/proc/${parent.pid}/task/${task}/children`)
    } catch (cause) {
      // A task can exit between task-directory enumeration and its children
      // read. Its child list is then incomplete, so admit none of this pass.
      if (isMissingProcess(cause)) return
      throw new Error(`failed to read children for process ${parent.pid} task ${task}`, { cause })
    }
    listed
      .trim()
      .split(/\s+/)
      .forEach((value) => {
        ...
        if (!Number.isSafeInteger(child) || child <= 0)
          throw new Error(`process ${parent.pid} task ${task} reported an invalid child PID`)
        children.add(child)
      })
  }
  const after = linuxProcessRecord(parent.pid)
  if (!after || !sameLinuxRecord(before, after) || !matchesLinuxIdentity(after, parent)) return
  return Array.from(children)
}

function linuxSnapshot(pid: number, includeZombie = false): ProcessSnapshot | undefined {
  const record = linuxProcessRecord(pid)
  if (!record) return
  // A zombie has no executable identity, but an already nonce-authorized
  // zombie is still owned until its parent reaps it. The synthetic executable
  // is never used to signal: it only lets discovery retain the PID/start-time
  // identity for finalization. Do not construct it before nonce admission.
  if (record.state === "Z") {
    if (!includeZombie) return
    return {
      pid,
...
const linuxProcObservation: LinuxProcObservation = {
  record: linuxProcessRecord,
  snapshot: linuxSnapshot,
}

function linuxProcessGroup(
  pid: number,
  read: (file: string, encoding: "utf8") => string = (file, encoding) => readFileSync(file, encoding),
) {
  // stat keeps a zombie's PGID after /proc/<pid>/exe disappears. It remains
  // material to drain: signal-0 must reach ESRCH only after that zombie has
  // been reaped, so never filter it from this membership observation.
  return linuxProcessRecord(pid, read)?.group
}

function linuxProcessGroupExists(group: number) {
  try {
    process.kill(-group, 0)
    return true
  } catch (cause) {
    if (typeof cause === "object" && cause !== null && "code" in cause && cause.code === "ESRCH") return false
    // EPERM still proves that a member exists; it only says this harness
    // cannot signal that member. Treat it as a live group, never as drain.
    if (typeof cause === "object" && cause !== null && "code" in cause && cause.code === "EPERM") return true
    throw cause
  }
}

// The nonce is only read for live, previously untrusted candidates. In
// particular, do not let an unrelated zombie or an inaccessible process make
...
function sameLinuxRecord(left: LinuxProcessRecord, right: LinuxProcessRecord) {
  return left.parent === right.parent && left.group === right.group && left.started === right.started
}

function matchesLinuxIdentity(record: LinuxProcessRecord, identity: ProcessSnapshot) {
  return record.group === identity.group && record.started === identity.started
}

// `/proc/<pid>/environ` is an admission mechanism, not a lifetime
// mechanism. Linux stops exposing it once a child becomes a zombie. A member
// which was admitted while live can still be observed safely by bracketing its
// PID, start token, and PGID with stat reads. The optional parent is also
// bracketed when admitting a previously unseen descendant by lineage.
function linuxTrustedSnapshot(
  pid: number,
  expected: LinuxProcessRecord,
  parent?: ProcessSnapshot,
  observation = linuxProcObservation,
): ProcessSnapshot | undefined {
  const parentBefore = parent ? observation.record(parent.pid) : undefined
  if (parent && (!parentBefore || !matchesLinuxIdentity(parentBefore, parent))) return
  const before = observation.record(pid)
  if (!before || !sameLinuxRecord(before, expected)) return
  if (parent && before.parent !== parent.pid) return
  const snapshot = observation.snapshot(pid, true)
  const after = observation.record(pid)
  const parentAfter = parent ? observation.record(parent.pid) : undefined
  if (
    !snapshot ||
    snapshot.parent !== before.parent ||
    snapshot.group !== before.group ||
    snapshot.started !== before.started ||
    !after ||
    !sameLinuxRecord(before, after)
  )
    return
  if (parent && (!parentAfter || !sameLinuxRecord(parentBefore!, parentAfter))) return
  return { ...snapshot, containment: "", nonce: true }
}

const linuxProcReader: LinuxProcReader = {
  entries: () => readdirSync("/proc"),
  record: linuxProcessRecord,
  children: linuxProcessChildren,
  hasNonce: linuxHasNonce,
  trustedSnapshot: linuxTrustedSnapshot,
}

function linuxNonceSnapshot(pid: number, nonce: string, reader = linuxProcReader) {
  // Probe the environment before touching stat. A PID can disappear or become
  // unreadable after /proc enumeration, and that arbitrary process has not
  // crossed this launch's ownership boundary. Recheck after stat so PID reuse
  // cannot combine one process's environment with another's identity.
  if (!reader.hasNonce(pid, nonce)) return
  const before = reader.record(pid)
  // A zombie cannot newly cross the nonce boundary: its environment is no
  // longer a trustworthy source. It can only be retained through a member
  // recorded while live or through an already trusted parent lineage.
  if (!before || before.state === "Z" || !reader.hasNonce(pid, nonce)) return
  return reader.trustedSnapshot(pid, before)
}
...
function trackPortableProcess(
  ...
  const tracked = new Map<number, ProcessIdentity>()
  // Keep the Linux admission record separately from the public identity map.
  // In particular, `ProcessIdentity` deliberately does not carry a PGID, but
  // a descendant which calls setpgid(2) needs that PGID retained after it has
  // become a zombie and lost its environment.
  const linuxOwned = new Map<number, ProcessSnapshot>()
  const errors: Error[] = []
  const cursor: DarwinScanCursor = { next: 0 }
  let awaitingGateExec = true
  let scanComplete = true
  const rememberLinux = (identity: ProcessIdentity) => {
    if (process.platform !== "linux") return
    if (
      !("group" in identity) ||
      !("parent" in identity) ||
      !("nonce" in identity) ||
      typeof identity.group !== "number" ||
      typeof identity.parent !== "number" ||
      typeof identity.nonce !== "boolean"
    )
      return
    linuxOwned.set(identity.pid, { ...identity, group: identity.group, parent: identity.parent, nonce: identity.nonce })
  }
  const snapshots = async (deadline: number) => {
    const result = await (
      options?.snapshots?.(deadline) ?? portableSnapshots(nonce, deadline, child.pid, cursor, Array.from(linuxOwned.values()))
    )
  ...
  const inspect = async (pid: number, deadline: number) => {
    ...
    if (options?.inspect) return options.inspect(pid, deadline)
    const known = linuxOwned.get(pid)
    if (process.platform === "linux" && known) {
      const current = linuxProcessRecord(pid)
      if (!current || !matchesLinuxIdentity(current, known)) return
      const snapshot = linuxTrustedSnapshot(pid, current)
      return snapshot ? { ...snapshot, containment: known.containment } : undefined
    }
    return portableSnapshot(pid, nonce, deadline, true)
  }
    ...
    try {
      ...
      for (const identity of tracked.values()) {
        try {
          ...
          if (!live) {
            // linuxSnapshot cannot form an executable identity for a zombie,
            // but a zombie is not gone. Retain this known member so the
            // controller finalizer must wait for its parent to reap it.
            const zombie =
              process.platform === "linux" &&
              (() => {
                const current = linuxProcessRecord(identity.pid)
                return current?.started === identity.started && current.state === "Z"
              })()
            if (zombie) {
  ...
  return serializeTracker({
    ...
    stop: async (deadline = Date.now() + 4_000) => {
      ...
      await finalizePortableController(options.controller, deadline, errors, Array.from(tracked.values()))
    },
...
// A signed record proves only who must be authenticated.  It must not let one
// uncooperative exact-PID probe consume the publication window (or the later
// containment budget).  Publication itself remains eligible for the full
// acquisition deadline; this slice begins only after that record is visible.
const portableControllerObservationSliceMs = 250
...
function portableControllerObservationDeadline(deadline: number, now = Date.now()) {
  return Math.min(deadline, now + portableControllerObservationSliceMs)
}
...
type PortableIdentityExit = {
  readonly exited: boolean
  readonly zombie: boolean
}

async function portableIdentityExited(
  identity: ProcessIdentity,
  deadline: number,
  errors: Error[],
  name: string,
): Promise<PortableIdentityExit> {
  try {
    if (Date.now() >= deadline) return { exited: false, zombie: false }
    if (process.platform === "linux") {
      const current = linuxProcessRecord(identity.pid)
      if (!current || current.started !== identity.started) return { exited: true, zombie: false }
      return { exited: false, zombie: current.state === "Z" }
    }
    const current = await darwinNativeSnapshot(identity.pid)
    if (!current || current.started !== identity.started) return { exited: true, zombie: false }
    return { exited: false, zombie: false }
  } catch (cause) {
...
type LinuxGroupDrainReader = {
  readonly entries: () => readonly string[]
  readonly groupExists: (group: number) => boolean
  readonly group: (pid: number) => number | undefined
}

const linuxGroupDrainReader: LinuxGroupDrainReader = {
  entries: () => readdirSync("/proc"),
  groupExists: linuxProcessGroupExists,
  group: linuxProcessGroup,
}

async function portableGroupDrained(
  group: number,
  deadline: number,
  errors: Error[],
  reader: LinuxGroupDrainReader = linuxGroupDrainReader,
) {
  try {
    if (process.platform === "linux") {
      if (Date.now() >= deadline) return false
      // A PGID scan is diagnostic and can race arbitrary processes. The
      // signal-0 probe is the authoritative membership test: ESRCH means no
      // process, including a zombie, remains in this owned group; EPERM means
      // a member still exists and therefore cannot be reported as drained.
      const groupExists = reader.groupExists(group)
      const memberObserved = reader
        .entries()
        .filter((entry) => /^\d+$/.test(entry))
        .some((entry) => {
          if (Date.now() >= deadline) return true
          try {
            // This intentionally reads only stat. An unrelated protected
            // executable must never make the owned group look undrained.
            return reader.group(Number(entry)) === group
          } catch {
            // An unreadable unrelated stat cannot weaken signal-0's exact
            // result. If it is our group, groupExists remains true.
            return false
          }
        })
      return !groupExists && !memberObserved
    }
...
// Exercise the production drain algorithm while instrumenting the exact
// unrelated /proc reads it performs. The drain only needs a PGID from stat;
// an unrelated executable is never an input to this decision. The owned child
// exits but remains a zombie until the target reaps it on TERM.
export async function portableLinuxZombieGroupDrainForTest() {
  if (process.platform !== "linux") throw new Error("Linux zombie group drain regression requires Linux")
  const compiler = Bun.which("cc")
  if (!compiler) throw new Error("Linux zombie group drain regression requires cc")
  const root = path.join(os.tmpdir(), `opencode-zombie-group-${crypto.randomUUID()}`)
  const source = `${root}.c`
  const executable = `${root}.bin`
  const zombieMarker = `${root}.zombie`
  const pendingZombieMarker = `${zombieMarker}.pending`
  let unrelated: ReturnType<typeof Bun.spawn> | undefined
  let proc: TrackedProcess | undefined
  let cleaned = false
  try {
    writeFileSync(
      source,
...
static void stop(int signal) { stopping = signal; }
static int publish_zombie(const char *marker, pid_t child) {
  char pending[4096];
  if (snprintf(pending, sizeof(pending), "%s.pending", marker) >= (int)sizeof(pending)) return 1;
  FILE *file = fopen(pending, "w");
  if (!file) return 1;
  if (fprintf(file, "%d", child) < 0 || fflush(file) != 0 || fsync(fileno(file)) != 0 || fclose(file) != 0) return 1;
  return rename(pending, marker) != 0;
}
int main(int count, char **values) {
  ...
  if (child < 0) return 5;
  if (child == 0) {
    if (setpgid(0, 0) != 0) _exit(8);
    _exit(0);
  }
  siginfo_t exited = {0};
  if (waitid(P_PID, child, &exited, WEXITED | WNOWAIT) != 0 || exited.si_pid != child || exited.si_code != CLD_EXITED) return 6;
  if (getpgid(child) != child) return 7;
  if (publish_zombie(values[2], child) != 0) return 9;
  while (!stopping) pause();
  return waitpid(child, 0, 0) == child ? 0 : 10;
}
`,
    )
    const built = Bun.spawnSync([compiler, source, "-o", executable])
    if (built.exitCode !== 0) throw new Error(`failed to compile Linux zombie group fixture: ${built.stderr.toString()}`)
    unrelated = Bun.spawn([process.execPath, "-e", "setInterval(() => {}, 1_000)"], {
      stdin: "ignore",
      stdout: "ignore",
      stderr: "ignore",
    })
    proc = await spawnTracked([executable, "owned", zombieMarker], { stdout: "pipe", stderr: "pipe" }, { forcePortable: true })
    const zombieDeadline = Date.now() + 1_000
    while (!existsSync(zombieMarker) && Date.now() < zombieDeadline) await Bun.sleep(5)
    if (!existsSync(zombieMarker)) throw new Error("owned Linux zombie fixture did not publish its descendant")
    const zombiePID = Number(readFileSync(zombieMarker, "utf8"))
    const zombie = linuxProcessRecord(zombiePID)
    if (!zombie || zombie.state !== "Z" || zombie.group !== zombiePID)
      throw new Error("owned Linux fixture descendant did not become a group zombie")
    const trackedZombie = (await proc.tracker.scan(Date.now() + 1_000)).some(
      (identity) => identity.pid === zombiePID && identity.started === zombie.started && identity.executable === "[zombie]",
    )
    if (!trackedZombie || proc.tracker.errors.length)
      throw new Error("portable discovery did not retain the owned zombie descendant")
    const errors: Error[] = []
    const unrelatedPaths: string[] = []
    const drainWhileZombie = await portableGroupDrained(zombie.group, Date.now() + 1_000, errors, {
      entries: () => [String(unrelated!.pid), String(zombiePID)],
      groupExists: linuxProcessGroupExists,
      group: (pid) =>
        linuxProcessGroup(pid, (file, encoding) => {
          unrelatedPaths.push(file)
          if (file.endsWith("/exe")) throw new Error(`drain read an unrelated executable path: ${file}`)
          return readFileSync(file, encoding)
        }),
    })
    if (drainWhileZombie || errors.length) throw new Error("portable group drain falsely accepted an owned zombie")
    const started = Date.now()
    await terminateProcessPromise(proc.child, () => {}, proc.tracker, Date.now() + 4_000, proc.rawExited)
    cleaned = true
    const cleanupDurationMs = Date.now() - started
    const drainedAfterCleanup = await portableGroupDrained(zombie.group, Date.now() + 1_000, errors)
    const reaped = linuxProcessRecord(zombiePID) === undefined
    return {
      cleanupDurationMs,
      drainWhileZombie,
      drainedAfterCleanup,
      reaped,
      trackedZombie,
      unrelatedExecutablePaths: unrelatedPaths.filter((file) => file === `/proc/${unrelated!.pid}/exe`),
      unrelatedStatRead: unrelatedPaths.includes(`/proc/${unrelated!.pid}/stat`),
    }
  } finally {
    if (proc && !cleaned)
      await terminateProcessPromise(proc.child, () => {}, proc.tracker, Date.now() + 4_000, proc.rawExited).catch(() => undefined)
    try {
      unrelated?.kill("SIGKILL")
    } catch (cause) {
      if (!isMissingProcess(cause)) throw cause
    }
    if (unrelated) await unrelated.exited.catch(() => undefined)
    ;[source, executable, zombieMarker, pendingZombieMarker].forEach((file) => {
      if (existsSync(file)) unlinkSync(file)
    })
  }
}

// Keep the single-thread fixture above: it verifies the original leader-task
// path. This fixture exercises Linux's per-thread child accounting, where a
// child forked by a worker appears only below that worker's task directory.
export async function portableLinuxNonLeaderZombieGroupDrainForTest() {
  if (process.platform !== "linux") throw new Error("Linux non-leader zombie regression requires Linux")
  const compiler = Bun.which("cc")
  if (!compiler) throw new Error("Linux non-leader zombie regression requires cc")
  const root = path.join(os.tmpdir(), `opencode-nonleader-zombie-${crypto.randomUUID()}`)
  const source = `${root}.c`
  const executable = `${root}.bin`
  const zombieMarker = `${root}.zombie`
  const pendingZombieMarker = `${zombieMarker}.pending`
...
static pid_t forker;
static void stop(int signal) { stopping = signal; }
static int publish_zombie(const char *marker, pid_t child, pid_t task) {
  char pending[4096];
  if (snprintf(pending, sizeof(pending), "%s.pending", marker) >= (int)sizeof(pending)) return 1;
  FILE *file = fopen(pending, "w");
  if (!file) return 1;
  if (fprintf(file, "%d:%d", child, task) < 0 || fflush(file) != 0 || fsync(fileno(file)) != 0 || fclose(file) != 0) return 1;
  return rename(pending, marker) != 0;
}
static void *fork_zombie(void *unused) {
  (void)unused;
  pid_t created = fork();
  if (created == 0) {
    if (setpgid(0, 0) != 0) _exit(8);
  ...
  child = created;
  forker = (pid_t)syscall(SYS_gettid);
  pthread_cond_signal(&ready);
  pthread_mutex_unlock(&lock);
  while (!stopping) usleep(1000);
  return NULL;
}
int main(int count, char **values) {
  if (count != 2) return 2;
  ...
  signal(SIGINT, stop);
  pthread_t worker;
  if (pthread_create(&worker, NULL, fork_zombie, NULL) != 0) return 3;
  pthread_mutex_lock(&lock);
  while (!child) pthread_cond_wait(&ready, &lock);
  pid_t created = child;
  pid_t task = forker;
  pthread_mutex_unlock(&lock);
  if (created < 0) return 4;
  siginfo_t exited = {0};
  if (waitid(P_PID, created, &exited, WEXITED | WNOWAIT) != 0 || exited.si_pid != created || exited.si_code != CLD_EXITED) return 5;
  if (getpgid(created) != created) return 6;
  if (publish_zombie(values[1], created, task) != 0) return 7;
  while (!stopping) usleep(1000);
  if (pthread_join(worker, NULL) != 0) return 8;
  return waitpid(created, 0, 0) == created ? 0 : 9;
}
`,
    )
    const built = Bun.spawnSync([compiler, source, "-pthread", "-o", executable])
    if (built.exitCode !== 0)
      throw new Error(`failed to compile Linux non-leader zombie fixture: ${built.stderr.toString()}`)
    proc = await spawnTracked([executable, zombieMarker], { stdout: "pipe", stderr: "pipe" }, { forcePortable: true })
    const zombieDeadline = Date.now() + 1_000
    while (!existsSync(zombieMarker) && Date.now() < zombieDeadline) await Bun.sleep(5)
    if (!existsSync(zombieMarker)) throw new Error("non-leader Linux zombie fixture did not publish its descendant")
    const [zombiePID = 0, forkerTID = 0] = readFileSync(zombieMarker, "utf8").split(":").map(Number)
    if (!Number.isSafeInteger(zombiePID) || !Number.isSafeInteger(forkerTID) || forkerTID === proc.child.pid)
      throw new Error("non-leader Linux fixture did not fork from a worker task")
    const leaderChildren = linuxBoundedRead(`/proc/${proc.child.pid}/task/${proc.child.pid}/children`)
      .trim()
      .split(/\s+/)
      .map(Number)
    if (leaderChildren.includes(zombiePID)) throw new Error("non-leader Linux child unexpectedly appeared under leader task")
    const zombie = linuxProcessRecord(zombiePID)
    if (!zombie || zombie.state !== "Z" || zombie.group !== zombiePID)
      throw new Error("non-leader Linux fixture descendant did not become a group zombie")
    const trackedZombie = (await proc.tracker.scan(Date.now() + 1_000)).some(
      (identity) => identity.pid === zombiePID && identity.started === zombie.started && identity.executable === "[zombie]",
    )
    if (!trackedZombie || proc.tracker.errors.length)
      throw new Error("portable discovery did not retain the non-leader zombie descendant")
    const errors: Error[] = []
    const drainWhileZombie = await portableGroupDrained(zombie.group, Date.now() + 1_000, errors)
    if (drainWhileZombie || errors.length) throw new Error("portable group drain falsely accepted a non-leader zombie")
    const started = Date.now()
    await terminateProcessPromise(proc.child, () => {}, proc.tracker, Date.now() + 4_000, proc.rawExited)
    cleaned = true
    const cleanupDurationMs = Date.now() - started
    const drainedAfterCleanup = await portableGroupDrained(zombie.group, Date.now() + 1_000, errors)
    const reaped = linuxProcessRecord(zombiePID) === undefined
    return { cleanupDurationMs, drainWhileZombie, drainedAfterCleanup, reaped, trackedZombie }
  } finally {
    if (proc && !cleaned)
      await terminateProcessPromise(proc.child, () => {}, proc.tracker, Date.now() + 4_000, proc.rawExited).catch(() => undefined)
    ;[source, executable, zombieMarker, pendingZombieMarker].forEach((file) => {
...
async function finalizePortableController(
  controller: PortableController,
  deadline: number,
  errors: Error[],
  ...
  const completion = async (limit: number): Promise<PortableControllerCompletion> => {
    const marker = state()
    const identities = [controller.identity, ...(controller.target ? [controller.target] : []), ...descendants]
    const exits = await Promise.all(
      identities.map((identity, index) =>
        portableIdentityExited(
          identity,
          limit,
          errors,
          index === 0 ? "controller" : index === 1 && controller.target ? "target" : "descendant",
        ),
      ),
    )
    const groupDrained = await portableGroupDrained(controller.identity.group, limit, errors)
    const controllerExited = exits[0]?.exited === true
    const targetExited = controller.target ? exits[1]?.exited === true : true
    const descendantExits = exits.slice(controller.target ? 2 : 1)
    const descendantsExited = descendantExits.every((exit) => exit.exited)
    return {
      ...marker,
      controllerExited,
      targetExited,
      descendantsExited,
      zombiePIDs: identities.filter((_, index) => exits[index]?.zombie).map((identity) => identity.pid),
      groupDrained,
      // kill-issued is only an intent record. A KILL finishes only once every
      // owned process was reaped and signal-0 reports the entire group gone.
      killFinished: marker.killIssued && controllerExited && targetExited && descendantsExited && groupDrained,
    }
  }
  ...
  // After the parent's identity-checked fallback KILL, use only the
  // separately reserved fallback interval for reaping and group-drain
  // verification. Waiting through the original controller grace again can
  // consume the whole cleanup budget while adding no containment authority.
  const final = await wait(
    completed?.acknowledged && completed.killFinished
      ? deadline
      : Math.min(deadline, Date.now() + portableControllerFallbackReserveMs),
  )
  ...
  if (!final.controllerExited)
    recordError(errors, `portable controller ${controller.identity.pid} did not reap after group KILL`, undefined)
  if (controller.target && !final.targetExited)
    recordError(errors, `portable target ${controller.target.pid} did not reap after group KILL`, undefined)
  if (!final.descendantsExited)
    recordError(errors, "portable descendants did not reap after group KILL", final.zombiePIDs.join(",") || undefined)
...
async function portableSnapshots(
  ...
  knownLinux: readonly ProcessSnapshot[] = [],
  options?: { readonly linux?: Partial<LinuxProcReader> },
): Promise<PortableSnapshots> {
  if (process.platform === "linux") {
    const reader: LinuxProcReader = { ...linuxProcReader, ...options?.linux }
    const errors: Error[] = []
    ...
    let complete = true
    const pids = reader.entries().flatMap((entry) => {
    ...
    const admitted = new Map<number, ProcessSnapshot>()
    knownLinux.forEach((identity) => {
      try {
        const record = reader.record(identity.pid)
        if (!record || !matchesLinuxIdentity(record, identity)) return
        const snapshot = reader.trustedSnapshot(identity.pid, record)
        if (!snapshot) return
        const owned = { ...snapshot, containment: identity.containment, nonce: true }
        snapshots.set(owned.pid, owned)
        admitted.set(owned.pid, owned)
      } catch (cause) {
        // This PID was admitted while live, so an opaque identity is material
        // to cleanup. Never apply this diagnostic to an untrusted entry.
        recordError(errors, `failed to snapshot nonce-authorized process ${identity.pid}`, cause)
      }
    })
    pids.forEach((pid) => {
      if (Date.now() >= discoveryDeadline) {
      ...
      if (snapshots.has(pid)) return
      try {
        const snapshot = linuxNonceSnapshot(pid, nonce, reader)
        if (!snapshot) return
        const owned = { ...snapshot, containment: `nonce:${nonce}`, nonce: true }
        snapshots.set(owned.pid, owned)
        admitted.set(owned.pid, owned)
      } catch (cause) {
        // We only report an opaque process after its exact nonce has already
        // authorized it. Failures for unrelated /proc entries stay tolerant.
        recordError(errors, `failed to snapshot nonce-authorized process ${pid}`, cause)
      }
    })
    // A zombie cannot authorize itself because Linux has already discarded
    // its environment. Discover one only through the kernel's direct-child
    // lists of every task in an admitted parent, then require a stable child
    // stat bracket
    // while the parent's creation identity is bracketed too. This avoids both
    // a broad stat walk before nonce admission and stale numeric PPID reuse.
    const examinedParents = new Set<number>()
    while (Date.now() < discoveryDeadline) {
      const parents = Array.from(admitted.values()).filter((parent) => !examinedParents.has(parent.pid))
      if (!parents.length) break
      parents.forEach((parent) => {
        examinedParents.add(parent.pid)
        if (Date.now() >= discoveryDeadline) {
          complete = false
          return
        }
        ...
        try {
          children = reader.children(parent)
        } catch (cause) {
          // The parent is owned, so an opaque child list leaves containment
          // uncertain. Do not turn individual arbitrary child PIDs into
          // ownership merely because this read failed.
          recordError(errors, `failed to inspect children of nonce-authorized process ${parent.pid}`, cause)
          complete = false
          return
        }
        if (!children) {
          // A parent can exit, recycle, or lose a task while its task lists
          // are observed. This is not evidence about any listed PID.
          complete = false
          return
        }
        children.forEach((pid) => {
          ...
          if (snapshots.has(pid)) return
          try {
            const record = reader.record(pid)
            // A live child must cross the normal nonce boundary. Only an
            // unobservable zombie may cross this lineage boundary.
            if (!record || record.state !== "Z" || record.parent !== parent.pid) return
            const snapshot = reader.trustedSnapshot(pid, record, parent)
            if (!snapshot || snapshot.parent !== parent.pid || snapshot.executable !== "[zombie]") return
            const owned = { ...snapshot, containment: parent.containment, nonce: true }
            snapshots.set(owned.pid, owned)
            admitted.set(owned.pid, owned)
          } catch {
            // This child has not completed its own stat identity bracket. It
            // is not safe to retain or diagnose as owned, even though a known
            // parent named its numeric PID.
          }
        })
      })
    }
    if (Date.now() >= discoveryDeadline) complete = false
    return { snapshots: Array.from(snapshots.values()), errors, complete }
  }
  if (process.platform !== "darwin")
    throw new Error(`portable process containment is unsupported on ${process.platform}`)
  const errors: Error[] = []
...
// A broad /proc enumeration must not touch stat before a process has proved it
// carries this launch's nonce. An opaque unrelated PID is not evidence about
// this launch at all.
export async function portableUnrelatedStatRaceForTest() {
  if (process.platform !== "linux") throw new Error("Linux /proc race regression requires Linux")
  let statReads = 0
  const result = await portableSnapshots("test", Date.now() + 1_000, undefined, undefined, [], {
    linux: {
      entries: () => ["4242"],
      record: () => {
        statReads += 1
        if (statReads === 1) return { parent: 1, group: 1, started: "test:4242", state: "S" }
        throw new Error("synthetic unreadable stat")
      },
      hasNonce: () => false,
    },
  })
  return { ...result, statReads }
}

// Once a readable nonce has authorized a previously unseen process, a failed
// stat bracket is material: returning an empty scan would incorrectly let the
// finalizer declare the owned process quiescent.
export async function portableOwnedUnreadableStatForTest() {
  if (process.platform !== "linux") throw new Error("Linux /proc race regression requires Linux")
  const result = await portableSnapshots("test", Date.now() + 1_000, undefined, undefined, [], {
    linux: {
      entries: () => ["4242"],
      hasNonce: () => true,
      record: () => {
        throw new Error("synthetic unreadable owned stat")
      },
    },
  })
  return result
...
// procfs reads are not record reads. A short read can end before a later
// private-group zombie PID, so retain every chunk through EOF before parsing
// or admitting any candidate child.
export async function portableChunkedZombieLineageForTest() {
  if (process.platform !== "linux") throw new Error("Linux /proc chunked child regression requires Linux")
  const parent = {
    pid: 42,
    parent: 1,
    group: 42,
  ...
  const childRecord = { parent: 42, group: 43, started: "test:43", state: "Z" } satisfies LinuxProcessRecord
  const chunks = [new TextEncoder().encode("4"), new TextEncoder().encode("3 ")]
  let reads = 0
  let childSnapshotAttempts = 0
  const result = await portableSnapshots("test", Date.now() + 1_000, undefined, undefined, [parent], {
    linux: {
      entries: () => [],
      children: (candidate) => {
        if (candidate.pid !== parent.pid) return []
        const listed = linuxBoundedReadFrom("synthetic children", (bytes, offset, length) => {
          reads += 1
          const chunk = chunks.shift()
          if (!chunk) return 0
          if (chunk.byteLength > length) throw new Error("synthetic short chunk exceeded bounded read")
      ...
      hasNonce: () => false,
      record: (pid) => {
        if (pid === parent.pid) return { parent: 1, group: 42, started: "test:42", state: "S" }
        return childRecord
      },
      trustedSnapshot: (pid) => {
        if (pid === child.pid) {
          childSnapshotAttempts += 1
          return child
        }
        return parent
      },
  ...
  return { ...result, reads, childSnapshotAttempts }
}

// Overflow must discard the entire list. In particular, the initial complete
// chunk must not make its children candidates before the final byte fails.
export async function portableChildrenOverflowForTest() {
  if (process.platform !== "linux") throw new Error("Linux /proc child overflow regression requires Linux")
  const parent = {
    pid: 42,
    parent: 1,
    group: 42,
  ...
  const chunks = [new Uint8Array(linuxProcChildrenReadBytes), new Uint8Array([32])]
  let reads = 0
  let childRecordAttempts = 0
  const result = await portableSnapshots("test", Date.now() + 1_000, undefined, undefined, [parent], {
    linux: {
      entries: () => [],
      children: (candidate) => {
        if (candidate.pid !== parent.pid) return []
        return linuxBoundedReadFrom("synthetic children", (bytes, offset, length) => {
          reads += 1
          const chunk = chunks.shift()
          if (!chunk) return 0
          if (chunk.byteLength > length) throw new Error("synthetic overflow chunk exceeded bounded read")
          bytes.set(chunk, offset)
          return chunk.byteLength
        })
          .trim()
      ...
      hasNonce: () => false,
      record: (pid) => {
        if (pid !== parent.pid) childRecordAttempts += 1
        return { parent: 1, group: 42, started: "test:42", state: "S" }
      },
      trustedSnapshot: () => parent,
    },
  })
  return { ...result, reads, childRecordAttempts }
}

// A zombie which has never had a readable environment is still owned when its
// direct parent was authenticated first and both identities complete the
// production lineage admission path.
export async function portableOwnedZombieLineageForTest() {
  if (process.platform !== "linux") throw new Error("Linux zombie lineage regression requires Linux")
  const parent = {
    pid: 42,
    parent: 1,
    group: 42,
  ...
  const result = await portableSnapshots("test", Date.now() + 1_000, undefined, undefined, [parent], {
    linux: {
      entries: () => [],
      children: (candidate) => (candidate.pid === parent.pid ? [43] : []),
      hasNonce: () => false,
      record: (pid) => {
        if (pid === parent.pid) return { parent: 1, group: 42, started: "test:42", state: "S" }
        childStatReads += 1
      ...
      trustedSnapshot: (pid, record, ancestor) => {
        if (pid === 43) {
          childSnapshotAttempts += 1
          return linuxTrustedSnapshot(pid, record, ancestor, {
            record: (candidate) => {
              if (candidate === parent.pid) return { parent: 1, group: 42, started: "test:42", state: "S" }
              childStatReads += 1
              return childRecord
            },
            snapshot: () => child,
  ...
  return { ...result, childSnapshotAttempts, childStatReads }
}

// A parent PID is not ownership. The child's own trusted observation must
// reject it when the parent identity bracket shows that PID was recycled.
export async function portableParentPIDReuseForTest() {
  if (process.platform !== "linux") throw new Error("Linux /proc race regression requires Linux")
  const parent = {
    pid: 42,
    parent: 1,
  ...
  const result = await portableSnapshots("test", Date.now() + 1_000, undefined, undefined, [parent], {
    linux: {
      ...
      trustedSnapshot: (pid, _record, ancestor) => {
        if (pid === 43) {
          childSnapshotAttempts += 1
          return linuxTrustedSnapshot(pid, childRecord, ancestor, {
            record: (candidate) => {
              if (candidate === 43) {
                childStatReads += 1
                return childRecord
              }
              parentBracketReads += 1
              return parentBracketReads === 1
                ? { parent: 1, group: 42, started: "test:42", state: "S" }
                : { parent: 1, group: 42, started: "test:reused", state: "S" }
            },
            snapshot: () => child,
          })
        }
  ...
  return { ...result, childSnapshotAttempts, childStatReads, parentBracketReads }
}

// A direct-child listing is only a candidate. A child stat failure before its
// own identity bracket cannot be admitted or diagnosed as owned.
export async function portablePreIdentityLineageFailureForTest() {
  if (process.platform !== "linux") throw new Error("Linux /proc race regression requires Linux")
  const parent = {
    pid: 42,
    parent: 1,
  ...
  const result = await portableSnapshots("test", Date.now() + 1_000, undefined, undefined, [parent], {
    linux: {
      ...
      record: (pid) => {
        if (pid === 43) {
          childRecordAttempts += 1
          throw new Error("synthetic unreadable child stat")
        }
        return { parent: 1, group: 42, started: "test:42", state: "S" }
      },
      trustedSnapshot: (pid) => {
        if (pid === 43) childSnapshotAttempts += 1
        return parent
      },
    },
  })
  return { ...result, childRecordAttempts, childSnapshotAttempts }
}

// The task directory is a snapshot boundary too. If a thread disappears while
// it is being enumerated, its former child list cannot authorize a recycled
// PID from a later process.
export async function portableParentTaskDisappearanceForTest() {
  if (process.platform !== "linux") throw new Error("Linux task discovery regression requires Linux")
  const parent = {
    pid: 42,
    parent: 1,
    group: 42,
  ...
  const result = await portableSnapshots("test", Date.now() + 1_000, undefined, undefined, [parent], {
    linux: {
      ...
      hasNonce: () => false,
      record: (pid) => {
        if (pid === 43) childRecordAttempts += 1
        return { parent: 1, group: 42, started: "test:42", state: "S" }
      },
      trustedSnapshot: () => parent,
    },
  })
  return { ...result, childRecordAttempts }
}

// An unreadable task list is containment uncertainty for an owned parent, not
// permission to inspect PIDs which happened to be named before that failure.
export async function portableParentTaskUnreadableForTest() {
  if (process.platform !== "linux") throw new Error("Linux task discovery regression requires Linux")
  const parent = {
    pid: 42,
    parent: 1,
  ...
  const result = await portableSnapshots("test", Date.now() + 1_000, undefined, undefined, [parent], {
    linux: {
      entries: () => ["43"],
      children: () => {
        throw new Error("synthetic unreadable task children")
      },
      hasNonce: () => false,
      record: (pid) => {
        if (pid === 43) childRecordAttempts += 1
...
type SpawnTrackedTestOptions = {
  ...
  readonly onPortableControlFiles?: (controls: readonly string[]) => void
...
async function spawnTracked(
  ...
  test?.onPortableControlFiles?.(controllerControls)
  let child: ReturnType<typeof Bun.spawn>
  try {
    child = Bun.spawn(
      [
  ...
  const captureController = async (deadline: number, retryStatusFailures = false) => {
    let controllerRecord = ""
    let failure: unknown
    let failureDeadline: number | undefined
    let signedStatusAvailable = false
    const currentDeadline = () => failureDeadline ?? deadline
    while (Date.now() < currentDeadline()) {
      ...
      try {
        ...
        signedStatusAvailable = true
        // Wait for a signed publication for the whole admission window, then
        // bound each exact-PID authentication separately. In particular,
        // observePortable races this deadline even when the underlying
        // observation ignores its AbortSignal.
        let captured: ProcessSnapshot | undefined
        try {
          captured = await observePortable(
            test?.portableObserve ?? portableSnapshot,
            controllerPID,
            controllerNonce,
            portableControllerObservationDeadline(currentDeadline()),
            true,
            "initial",
          )
        } catch (cause) {
          // An observation failure does not authenticate the controller, but
          // it can be a transient /proc race just after exec. Keep retrying
          // the same signed status PID without weakening admission, but do
          // not lend finalization's reserve to a persistently bad status.
          failure = cause
          if (!retryStatusFailures) failureDeadline ??= portableControllerRecoveryDeadline(deadline)
          await Bun.sleep(5)
          continue
        }
        ...
        if (!retryStatusFailures && !isMissingProcess(cause)) throw cause
        failure = cause
        // A missing status file is normal while the controller is starting.
        // Do not trade the four-second publication window for cleanup reserve
        // until a signed record has been observed and authentication failed.
        if (!retryStatusFailures && signedStatusAvailable)
          failureDeadline ??= portableControllerRecoveryDeadline(deadline)
        await Bun.sleep(5)
      }
    }
  ...
  const abort = async (cause: unknown) => {
    ...
    const provisionalController =
      !tracker && controller
        ? {
            ...
            acknowledgement,
            completion,
            controls: [...controllerControls, ...(controllerPause ? [controllerPause] : [])],
            filesystem,
    ...
    // Before a controller status exists, the gate itself only exits after its
    // controller disappears. Reap the exact gate handle and verify that its
    // dedicated group is gone before dropping control files; otherwise an
    // uncertain controller retains its abort path for a later owner.
    const prePublicationResolved =
      !tracker &&
      !controller &&
      exitState === "exited" &&
      (await portableGroupDrained(child.pid, cleanupDeadline, errors))
    if (prePublicationResolved)
      controllerControls.forEach((file) => {
        try {
          if (filesystem.exists(file)) filesystem.unlink(file)
        } catch (cleanup) {
          recordError(errors, `failed to remove portable pre-publication control ${file}`, cleanup)
        }
      })
  ...
  try {
    ...
    // A signed status PID that stays unreadable or unauthenticatable is not
    // admission. Classify that failure promptly so abort and finalization
    // retain their independent containment reserve instead of waiting out the
    // four-second launch deadline.
...
export async function portableFilesystemFailureForTest(kind: "read" | "unlink") {
  ...
  } satisfies SpawnTrackedTestOptions
  if (kind === "unlink") {
...
export async function portableDelayedControllerHandshakeForTest(publicationDelayMs = 500) {
  const marker = path.join(os.tmpdir(), `opencode-delayed-controller-target-${crypto.randomUUID()}`)
  const started = Date.now()
  let admittedAt = 0
  let gatePID = 0
  let controllerObservationDeadline = 0
  let controllerObservationStartedAt = 0
  ...
  try {
    const proc = await spawnTracked(
      ...
      {
        forcePortable: true,
        portableControllerPublicationDelayMs: publicationDelayMs,
        portableObserve: async (pid, nonce, deadline, confirmedNonce, stage) => {
          // The fake records the exact deadline passed to the first signed
          // controller observation. It does not infer admission from elapsed
          // wall time, which makes the old recovery-cutoff regression
          // observable without timing assertions in the caller.
          if (stage === "initial" && pid !== gatePID) {
            controllerObservationStartedAt = Date.now()
            controllerObservationDeadline = deadline
          }
          return portableSnapshot(pid, nonce, deadline, confirmedNonce)
        },
    ...
    return {
      ...
      controllerObservationHadTime: controllerObservationDeadline > controllerObservationStartedAt,
      targetExecuted,
...
// Keep the former 2.6s boundary regression out of the scheduler. This is the
// same admission-time calculation used after a status publication, with a
// clock and publication instant supplied explicitly: a healthy publication
// may use all four seconds, while its first exact-PID observation is bounded
// independently from that instant.
export function portableControllerPublicationWindowForTest() {
  const started = 10_000
  const acquisitionDeadline = started + 4_000
  const predecessorCleanupCutoff = started + 2_500
  const publishedAt = started + 2_600
  return {
    publishedAt,
    predecessorCleanupCutoff,
    acquisitionDeadline,
    observationDeadline: portableControllerObservationDeadline(acquisitionDeadline, publishedAt),
  }
}
...
export async function portableControllerStartupFailureForTest() {
  ...
  const groupDrained = gatePID > 0 && (await portableGroupDrained(gatePID, Date.now() + 1_000, groupErrors))
  return {
    cause,
    reaped,
    groupDrained,
    controlsRemoved: controls.length > 0 && controls.every((file) => !existsSync(file)),
  ...
  const cause = await spawnTracked([process.execPath, "-e", target], { stdout: "pipe", stderr: "pipe" }, {
    ...
    portableObserve: async (pid, nonce, deadline, confirmedNonce, stage, signal) => {
      ...
      if (stage === "reconcile") {
        ...
        throw new Error("synthetic post-exec cleanup trigger")
      }
      if (stage === "cleanup")
...
async function waitForPortableMarker(marker: string, proc: TrackedProcess, deadline: number) {
  while (!existsSync(marker)) {
    const remaining = deadline - Date.now()
    if (remaining <= 0) throw new Error(`portable fixture did not publish ${path.basename(marker)} before its deadline`)
    const outcome = await Promise.race([
      Bun.sleep(Math.min(5, remaining)).then(() => "poll" as const),
      (proc.rawExited ?? proc.child.exited).then(
        () => "exited" as const,
        () => "exited" as const,
      ),
    ])
    if (outcome === "exited") throw new Error(`portable fixture exited before publishing ${path.basename(marker)}`)
  }
}

// A Bun timeout only stops the test runner. This deliberately exits the
// target before any marker can be published and proves each polling caller's
// immediate finally owns the already-acquired controller and its controls.
export async function portableMarkerPollingTeardownForTest() {
  const marker = path.join(os.tmpdir(), `opencode-portable-missing-marker-${crypto.randomUUID()}`)
  let controls: readonly string[] = []
  const proc = await spawnTracked([process.execPath, "-e", "process.exit(0)"], { stdout: "pipe", stderr: "pipe" }, {
    forcePortable: true,
    onPortableControlFiles: (files) => {
      controls = files
    },
  ...
  let cause: unknown
  try {
    await waitForPortableMarker(marker, proc, Date.now() + 1_000)
    throw new Error("portable marker polling fixture unexpectedly published a marker")
  } catch (error) {
    cause = error
  } finally {
    await terminateProcessPromise(proc.child, () => {}, proc.tracker, Date.now() + 4_000, proc.rawExited).catch(() => undefined)
  ...
  const reaped = await exitedWithin(
    proc.child,
    1_000,
    "after marker polling fixture teardown",
    [],
    proc.rawExited,
  )
  const groupErrors: Error[] = []
  return {
    cause,
    reaped,
    groupDrained: await portableGroupDrained(proc.child.pid, Date.now() + 1_000, groupErrors),
    controlsRemoved: controls.length > 0 && controls.every((file) => !existsSync(file)),
  }
}
...
export async function portableCompletionBeforeKillFallbackForTest() {
  ...
  try {
    ...
    termination = terminateProcessPromise(proc.child, () => {}, proc.tracker, Date.now() + 4_000, proc.rawExited)
    await waitForPortableMarker(controllerControls.completion, proc, Date.now() + 2_000)
    ...
    await (termination ?? terminateProcessPromise(proc.child, () => {}, proc.tracker, Date.now() + 4_000, proc.rawExited)).catch(
      () => undefined,
    )
    if (existsSync(marker)) unlinkSync(marker)
  }
...
async function portableControllerRecoveryForTest(
  ...
  const started = Date.now()
  let proc: TrackedProcess | undefined
  let launched: ReturnType<typeof Bun.spawn> | undefined
  let cause: unknown
  let rootReaped = false
  let transientAuthenticationFailure: Error | undefined
  const teardownErrors: Error[] = []
  try {
    proc = await spawnTracked([process.execPath, "-e", "setInterval(() => {}, 1_000)"], { stdout: "pipe", stderr: "pipe" }, {
      forcePortable: true,
      ...
      portableObserve: async (pid, nonce, deadline, confirmedNonce, stage) => {
        if (kind === "stall" && stage === "initial") {
          attempts++
          return new Promise<ProcessSnapshot>(() => {})
        }
        if (kind === "authenticate" && stage === "initial" && attempts++ < failures) {
          const error = new Error("synthetic controllerStatus authentication failed")
          transientAuthenticationFailure ??= error
          throw error
        }
        return portableSnapshot(pid, nonce, deadline, confirmedNonce)
      },
    ...
    // A transient admission retry can succeed. Exercise its independently
    // owned teardown instead of accidentally returning the live fixture.
    cause = await terminateProcessPromise(proc.child, () => {}, proc.tracker, Date.now() + 4_000, proc.rawExited).catch(
      (error) => error,
    )
  } catch (error) {
    cause = error
  } finally {
    const child = proc?.child ?? launched
    if (child) {
      rootReaped = await exitedWithin(
        child,
        1_000,
        "after controller recovery fixture teardown",
        teardownErrors,
        proc?.rawExited,
      )
      if (!rootReaped) {
        try {
          ...
          if (!isMissingProcess(error)) recordError(teardownErrors, "failed to force controller recovery fixture reaping", error)
        }
        rootReaped = await exitedWithin(
          child,
          1_000,
          "after forced controller recovery fixture teardown",
          teardownErrors,
          proc?.rawExited,
        )
      }
  ...
  if (teardownErrors.length)
    cause = new AggregateError(
      [
        ...(cause instanceof Error ? [cause] : [new Error("controller recovery fixture did not report cleanup failure")]),
        ...teardownErrors,
      ],
      "failed to reap portable controller recovery fixture",
    )
  if (transientAuthenticationFailure && cause instanceof Error)
    cause = new AggregateError(
      [transientAuthenticationFailure, cause],
      "portable controller recovery fixture failed after transient authentication",
    )
  return {
    cause,
    fallback,
...
// A signed controller record cannot let a hung exact-PID observation consume
// the original four-second publication budget. This deliberately ignores the
// AbortSignal so observePortable's own deadline is the only containment path.
export function portableStalledControllerRecoveryForTest() {
  return portableControllerRecoveryForTest("stall", Number.POSITIVE_INFINITY)
}
...
// Cleanup must not let a failed discovery or signal short-circuit the later
// escalation, direct-child reap, tracker stop, or the single aggregate report.
export async function terminationStepFailuresForTest(mode: "sync" | "reject") {
  let stopped = false
  let reaped = false
  ...
  const fail = (step: string) => {
    const error = new Error(`synthetic ${step} ${mode} failed`)
  ...
  const tracker: ProcessTracker = {
    ...
    signal: (signal) => {
      signals.push(signal)
      if (signal === "SIGKILL") resolveExit?.(0)
      return fail(`${signal} signal`)
    },
    stop: () => {
      stopped = true
      return fail("tracker stop")
    },
    errors: [],
  }
  const cause = await terminateProcessPromise(child, () => {}, tracker, Date.now() + 100).catch((error) => error)
  return { cause, exitObserved, reaped, signals, stopped }
}
...
async function terminateProcessPromiseUnlocked(
  ...
  const owned = async () => {
    ...
    try {
      ...
      return { processes, failed: false }
    } catch (cause) {
      collectCleanupErrors(errors, new Error(`failed to scan process tree for ${child.pid}`, { cause }))
      if (tracker.errors.length !== priorErrors) errors.push(...tracker.errors.slice(priorErrors))
      return { processes: [] as readonly ProcessIdentity[], failed: true }
    }
  }
  const goneWithin = async (
    ...
    let emptyScans = 0
    while (true) {
      const scan = await owned()
      if (resignal && (scan.failed || scan.processes.length)) await signal(resignal)
      ...
      if (
        !scan.failed &&
        (tracker.abort || tracker.errors.length === 0) &&
        tracker.scanComplete?.() !== false &&
  ...
  const signal = async (name: "SIGTERM" | "SIGKILL", limit = containmentDeadline) => {
    ...
    try {
      ...
      collectCleanupErrors(errors, new Error(`failed to send ${name} during process tree cleanup for ${child.pid}`, { cause }))
    }
    errors.push(...tracker.errors.slice(priorErrors))
    try {
      mark("exit.wait", child.pid)
    } catch (cause) {
      collectCleanupErrors(errors, new Error(`failed to mark ${name} during process tree cleanup for ${child.pid}`, { cause }))
    }
  }
  const reap = async (stage: string, limit = containmentDeadline) => {
    ...
    try {
      return await exitedWithin(child, remaining, stage, errors, observed)
    } catch (cause) {
      collectCleanupErrors(errors, new Error(`failed to reap process ${child.pid} ${stage}`, { cause }))
      return false
    }
  }
