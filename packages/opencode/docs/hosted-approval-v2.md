...
`config_generation_<32 lowercase hex>`. Each session observed by the permission
service also owns a random `session_incarnation_<32 lowercase hex>` that changes
after deletion/recreation and across process restarts. The generation changes after successful
...
Each insertion receives a random `request_incarnation_<32 lowercase hex>` and is
bound to its session incarnation.
...
effect. Missing, replayed, wrong-session, stale-session-incarnation,
wrong-request-incarnation, or digest mismatches
